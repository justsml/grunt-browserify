...
var log = require('debug')('grunt-browserify:runner');

module.exports = GruntBrowserifyRunner;

function GruntBrowserifyRunner(options) {
  log('Starting', Object.keys(options).sort().join(', '));
...
GruntBrowserifyRunner.prototype = _.create(GruntBrowserifyRunner.prototype, {
  run: function (files, destination, options, next) {
    ...
    b.on('error', function (err) {
      log('browserify:onERROR', destination, options, err, err.stack);
    ...
    if(options.require) {
      ...
      log('applied option: `require`=', options.require);
    }
    ...
    if (options.exclude) {
      ...
      log('applied option: `exclude`=', options.exclude);
    }
    ...
    if (options.ignore) {
      ...
      log('applied option: `ignore`=', options.ignore);
    }
    ...
    if (options.external) {
      ...
      log('applied option: `external`=', options.external);
    }

    if (options.transform) {
      _.forEach(options.transform, function (transformer) {
        b.transform(transformer);
        log('applied a `transform` ', transformer, '  serialized=', JSON.stringify(transformer));
        var txFn = Array.isArray(transformer) ? transformer[0] : transformer;
        txfn = typeof(txFn) === 'function' ? Object.getPrototypeOf(txFn) : txFn;
        log('    transform function prototype:', txFn);
      });
    }
    ...
    if (options.plugin) {
      ...
      log('applied option: `plugin`=', options.plugin);
    }
    ...
    if (options.watch) {
      ...
      log('watch enabled! keepAlive=', options.keepAlive);
    }
  ...
  onBundleComplete: function (destination, options, next) {
    ...
    return function (err, buf) {
      if (err) {
        log('onBundleComplete:ERROR', destination, options, err, err.stack);
        ...
        log('Completed bundle `%s` - %d KB', destination, (buf.length / 1024.0).toFixed(2));
      }
...
function doBundle(browserifyInstance, opts, bundleComplete) {
  ...
  browserifyInstance.bundle(function (err, buf) {
    if (err) {
      log('bundle:ERROR', opts, err, err.stack);
    }
